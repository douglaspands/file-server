...
#### Scenario: Suíte de 8 prompts mestres de fundação por linguagem e harness (Go, Python, Node.js e TypeScript para AGY e Claude Code)
- **WHEN** um desenvolvedor, PO ou QA consultar e utilizar os documentos sob `docs/generic-*-foundation-spec-prompt-*.md`
- **THEN** cada documento deve fornecer um prompt mestre parametrizável e agnóstico de frameworks para o respectivo par de ecossistema (Go, Python, Node.js ou TypeScript) e harness CLI (Antigravity ou Claude Code), sem decisões pré-moldadas de stack ou empacotamento (Pilar 5), com recomendação formal do framework OpenSpec e diretrizes de governança para PO e QA (Pilar 7), parametrização de plataformas alvo de compilação/release com fallback seguro (Pilar 8), e cumprimento rigoroso dos 10 pilares de excelência de engenharia

#### Scenario: Catálogo e Guia de Spec-Driven Development no README de Docs
- **WHEN** um desenvolvedor acessar o diretório `docs/`
- **THEN** o arquivo `docs/README.md` deve apresentar a visão geral do estudo de Spec-Driven Development (SDD) com OpenSpec, detalhar as disciplinas de Engenharia de Agentes e disponibilizar uma tabela descritiva e comparativa de todos os 8 prompts mestres da pasta

#### Scenario: Governança Git com .gitignore Mandatório e Squash Merge
- **WHEN** um novo projeto for iniciado ou governado pelos prompts mestres
- **THEN** o projeto deve conter um arquivo `.gitignore` completo cobrindo segredos (`.env`, chaves), binários compilados, relatórios de cobertura, caches de ferramentas e dependências locais, além de seguir estritamente o fluxo de feature branches por especificação e commits squash com autorização explícita
