...
Além disso, a aplicação terá identidade visual oficial com ícone de alta resolução embutido no executável binário e renderizado na barra de tarefas e janela desktop, tematização escura na moldura da janela do navegador/Chrome nas cores da aplicação (`slate-950` / `#020617`), normalização de versão limpa (sem duplicar a letra `"v"` de tags do Git), e o `README.md` apresentará a interface gráfica como o método principal de utilização.
...
- Aplicar tema escuro e paleta de cores (`#020617`) à moldura da janela do Chrome/Edge via Web App Manifest, meta tags de tema e flags CLI (`--force-dark-mode`).
...
### 2. Estilização da Janela do Navegador (Chrome App Mode, Meta Tags e Manifest)
- **Decisão**:
  - Fornecer `web/static/manifest.json` com `theme_color: "#020617"`, `background_color: "#020617"` e `display: "standalone"`.
  - Inserir tags `<meta name="theme-color" content="#020617">` e `<meta name="color-scheme" content="dark">` nos templates HTML (`gui_launcher.html` e `base.html`).
  - No `internal/adapters/gui/window.go`, ao invocar navegadores baseados em Chromium (Chrome, Chromium, Brave, Edge), incluir flags `--force-dark-mode` e `--enable-features=WebUIDarkMode`.
- **Racional**: Proporciona acabamento visual uniforme, garantindo que a barra de título e controles do navegador reflitam a paleta escura do tema Adwaita Dark.

### 3. Identidade Visual e Embutimento Multiplataforma do Ícone da Aplicação
- **Decisão**:
...
### 4. Normalização da Exibição de Versão
- **Decisão**:
...
### 5. Tratamento Escalável de Múltiplos IPs & Categorização de Interfaces
- **Decisão**: 
...
### 6. Estratégia de Documentação no README com Prints Visuais
- **Decisão**:
...
### 7. Diálogo Nativo de Seleção de Diretórios (Folder Picker)
...
### 8. Orquestração e Ciclo de Vida dos Servidores
...
### 9. Transmissão de Logs em Tempo Real
