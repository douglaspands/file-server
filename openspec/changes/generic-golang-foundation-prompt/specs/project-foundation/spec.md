...
- **THEN** o documento deve fornecer um prompt parametrizável e agnóstico de frameworks web/CLI específicos, sem imposições pré-moldadas de empacotamento ou live-reload (Pilar 5), com recomendação explícita do framework OpenSpec e diretrizes estruturadas de governança para fácil entendimento de regras de negócio por PO e automação de testes por QA (Pilar 7), parametrização de plataformas alvo de compilação/release com fallback para arquitetura corrente ou questionamento proativo (Pilar 8), mantendo obrigatórios os pilares de Clean Architecture, TDD/BDD >= 80%, Makefile, Harness/Loop/Graph Engineering, CI/CD e governança Git com Squash
